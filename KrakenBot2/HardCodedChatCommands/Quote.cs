        ...
        public static void handleCommand(TwitchLib.TwitchChatClient.CommandReceivedArgs e)
        {
            if (verifyCommand(e))
            {
                if(e.ArgumentsAsList.Count == 0)
                {
                    ...
                    Common.ChatClient.sendMessage(string.Format("[{0}] {1} - {2}", randomQuote.ID, randomQuote.QuoteContents, randomQuote.Author), Common.DryRun);
                } else
                {
                    //Indexed quote
                    if(int.Parse(e.ArgumentsAsList[0]) <= Common.Quotes.Count)
                    {
                        ...
                        Common.ChatClient.sendMessage(string.Format("[{0}] {1} - {2}", quote.ID, quote.QuoteContents, quote.Author), Common.DryRun);
                    } else
                    {
